...
use self::process::{client_name, detect_version, require_supported};
...
fn configure(paths: &Paths, arguments: ClientArgs) -> Result<Value> {
    ...
    require_supported(arguments.client, &version)?;
...
#[cfg(test)]
use self::configuration::configure_codex_approvals;
#[cfg(test)]
use self::files::{
    array_entry, backup, object_entry, path_text, paths_to_strings, read_json, restore, write_json,
};
#[cfg(test)]
use self::process::{command, output_with_timeout, replace_cli_server, version_parts};
